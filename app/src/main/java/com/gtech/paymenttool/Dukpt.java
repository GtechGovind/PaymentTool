...
public final class Dukpt {
     ...
     */
    private static byte[] computeKey(byte[] baseDerivationKey, byte[] keySerialNumber, BitSet keyRegisterBitmask, BitSet dataVariantBitmask) throws Exception {
     ...
     */
    private static BitSet getIpek(BitSet key, BitSet ksn, BitSet keyRegisterBitmask) throws Exception {
    ...
    public static byte[] encryptTripleDes(byte[] key, byte[] data, boolean padding) throws Exception {
        ...
        IvParameterSpec iv = new IvParameterSpec(new byte[8]);
        SecretKey encryptKey = SecretKeyFactory.getInstance("DESede").generateSecret(new DESedeKeySpec(key));
        Cipher encryptor;
        if (padding) {
    ...
    public static byte[] decryptTripleDes(byte[] key, byte[] data, boolean padding) throws Exception {
        ...
        if (padding) cipher = Cipher.getInstance("DESede/CBC/PKCS5Padding");
        else cipher = Cipher.getInstance("DESede/CBC/NoPadding");
